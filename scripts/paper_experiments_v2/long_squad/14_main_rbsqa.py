#!/usr/bin/env python3
"""
Long SQuAD v2 - RBSQA Main (Reasoning-Belief State Architecture)
==================================================================
...
- Progressive training for long documents

Key differences from 08_main_8tokens.py:
- Uses RBSXLNetForQA instead of standard memory
...
def create_config():
    ...
    return RBSTrainingConfig(
        ...
        memory_update="gated",
        memory_init="learned",
        # ✅ RBSQA: Use belief state architecture instead of standard memory
        use_rbsqa=True,
        belief_state_dim=64,  # Belief state dimension
        max_reasoning_steps=8,  # More steps for long documents
        halting_threshold=0.85,  # Lower threshold for more thorough reasoning on long docs
...
def main():
    print("\n" + "=" * 80)
    print("📊 EXPERIMENT RBSQA: LONG SQUAD V2 - MAIN (REASONING-BELIEF STATE ARCHITECTURE)")
    print("=" * 80 + "\n")
    print("🔧 Configuration details:")
    print("   ✅ memory_impl: 'token' → 'rbsqa_belief_state'")
    print("   ✅ use_rbsqa: False → True")
    print("   ✅ Added: belief_state_dim=64, max_reasoning_steps=8 (more for long docs)")
    print("   ✅ Added: halting_threshold=0.85 for adaptive reasoning")
                ...
                for time_step, batch in enumerate(time_step_batches):
                    ...
                    for ex_id, active in zip(batch["example_ids"], document_mask.tolist()):
                        if active:
                            ...
                            # 🔧 CRITICAL FIX: For long documents, use even stricter limit and cleanup in both modes
                            MEMORY_BANK_LIMIT = 2000  # Further reduced for long documents
